...
(() => {
  ...
  // src/content.ts
  import_webextension_polyfill.runtime.onMessage.addListener((msg) => {
    window.postMessage(msg, "*");
  });
  window.addEventListener("message", ({ data }) => {
    const { type } = data || {};
    if (type in Event) {
      import_webextension_polyfill.runtime.sendMessage(data).catch(() => {
      });
    }
  });
  if (!isProd || isFirefox) {
    const script = document.createElement("script");
    script.src = import_webextension_polyfill.runtime.getURL(isRateTest ? "page/rate.js" : "page/index.js");
