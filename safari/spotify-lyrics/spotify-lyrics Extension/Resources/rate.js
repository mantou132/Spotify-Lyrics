...
(() => {
  ...
  function raw(arr, ...args) {
    return arr.reduce((prev, current, index2) => prev + (args[index2 - 1] || "") + current);
  }
  var svg = raw;
  var css = raw;
  ...
  async function fetchNetEaseChineseName(s, fetchOptions) {
    const { API_HOST: API_HOST2 } = await configPromise;
    const searchQuery = new URLSearchParams({
      keywords: s,
      type: "100",
      limit: "100"
    });
    return request(`${API_HOST2}/search?${searchQuery}`, fetchOptions);
  }
  var down = 0;
  async function fetchNetEaseSongList(s, fetchOptions) {
    ...
    try {
      const res = await request(`${API_HOST2}/search?${searchQuery}`, fetchOptions);
  ...
  async function fetchNetEaseLyric(songId, fetchOptions) {
    const { API_HOST: API_HOST2 } = await configPromise;
    return request(`${API_HOST2}/lyric?${new URLSearchParams({ id: String(songId) })}`, fetchOptions);
  }
  ...
  function tryParseTimeStr(str) {
    ...
    const [min, sec] = [parseFloat(key2), parseFloat(value)];
    return { key: key2, value, time: min * 60 + sec };
  }
  function parseLyrics(lyricStr, options2 = {}) {
    ...
    const lyrics = lines.map((line) => {
      ...
      return matchResult.map((slice) => {
        ...
        const { key: key2, value, time } = tryParseTimeStr(matchResult2?.[0]);
        if (!isNaN(time)) {
        ...
        } else if (!options2.cleanLyrics && key2 && value) {
          result.text = `${key2.toUpperCase()}: ${value}`;
        }
        return result;
      });
    }).flat().sort((a, b) => {
    ...
    }).filter(({ text }, index2, arr) => {
      if (index2 === 0) {
        if (text === "") {
          return false;
        }
      } else {
        const prevEle = arr[index2 - 1];
  ...
  var index = playlists.findIndex((playlist) => playlist.link === location.pathname);
  var key = "spotify.lyrics.test";
  var listPromise = new Promise((res, rej) => {
    const fn = () => {
